...
module CC2420ThreatDetectionP {
	...
	uses interface CC2420Failures; // modified to signal crc errors
	...
	async event void CC2420Failures.crcFail(message_t *data){
	...
	async command error_t TX.transmit(ieee154_txframe_t *frame, uint32_t t0, uint32_t dt){
		...
		if(type == FRAMETYPE_BEACON){
			...
			if(call CCA.getNow() == FALSE){ // anomaly detected on beacon transmission
